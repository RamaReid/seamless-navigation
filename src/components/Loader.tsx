...
export const Loader: React.FC<LoaderProps> = ({ onComplete, isNavSkip = false }) => {
    ...
    const checkAssetsReady = async () => {
      try {
        ...
        const checkLayoutStability = () => {
          ...
          const container = heroShell || appLayer || document.body;
  ...
  useEffect(() => {
    ...
    // Fallback: verificar si el iframe tiene contenido
  ...
  // ========================================
  useEffect(() => {
    if (phase !== 'loading' || timedOut) return;
  ...
  }, [phase, timedOut, loaderCycles, assetsReady, revistaReady, requiredCycles]);

  // ========================================
  // GATE FINAL: Evaluar las 3 condiciones
  // ========================================
  useEffect(() => {
    if (phase !== 'loading') return;
    if (gateReadyFiredRef.current) return;
    
    const cyclesOk = loaderCycles >= requiredCycles;
  ...
  // Centro del reveal (centro de pantalla)
